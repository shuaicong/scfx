...
**结论：全串行，不并行。** 原因：每日增量仅 20 次 API 调用，串行 40s 完成，并行没有任何收益，反而增加 IP 封禁风险和代码复杂度。

**耗时估算：**

| 阶段 | 请求数 | 串行耗时 | 并行 3 路耗时 | 选择 |
|------|--------|---------|-------------|------|
| 每日增量（getPriceInfo） | 5 品种 × 4 区域 ≈ 20 次 | ~40s | ~15s | ✅ 串行（40s 已足够快） |
| 历史回填（getPriceChart） | ~150 组合 | ~5min | ~2min | ✅ 串行（减少 IP 风险） |
...
| API 请求间隔 | ≥ 1s，每个请求完成后等待 1s 再发下一个 | 防 IP 封禁 |
