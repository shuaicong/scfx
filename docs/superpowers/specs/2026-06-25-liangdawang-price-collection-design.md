...
| 约束 | 规则 | 示例 |
|------|------|------|
| 品种范围 | `variety` 参数限定：玉米/小麦/进口粮/国产大豆/生猪，LLM 无法匹配合法品种时**不调用工具**，走 RAG 或回复"暂不支持该品种查询" | 传入"稻谷"不调用工具，不走 SQL，改为 RAG 搜索或明确告知不支持 |
| 品种幻觉防护 | 在 system prompt 末尾追加：`注意：如果品种不在[玉米/小麦/进口粮/国产大豆/生猪]中，回答"暂不支该品种价格查询"，不要编造数据。` | LLM 不会从训练数据中编造虚假价格 |
