...
- **唯一键**：`(date, variety, region, area_type, source)` 联合唯一（同一天同一港口既有平仓价又有深加工收购价，属于两条有效行情，area_type 区分后不会互相覆盖）
...
- **回填与每日采集互斥：** 回填期间不执行每日采集。进度文件中标记 `backfill_completed=true` 后才允许 cron 触发正常采集。
  如果回填未完成时 cron 触发，`collect()` 检测到 `_is_first_run()=true` 且进度文件不完整，跳过本次采集，等下一次 cron 触发。
...
> 回填与日采互斥的分布式锁：单机时靠 JSON 进度文件中的 `backfill_completed` 标记；多实例部署时需引入 Redis 分布式锁。
> 锁 key: `lock:liangdawang:backfill`，过期时间 600s，竞争失败等待下一次 cron 触发。
> 单实例阶段不做 Redis 锁，部署超过 1 台时启用。
...
> **核心保障：** `ON DUPLICATE KEY UPDATE` 唯一键 `(date, variety, region, area_type, source)` 确保即使同一个组合被执行多次也不会产生重复行。
...
| **熔断恢复** | 熔断后自动等待 30 分钟解除，或运维手动重置（重启采集器）。同一机器多数据源共用 IP，任一数据源触发 429 熔断后，其他数据源也应暂停采集避免共享 IP 被封 |
| **告警收敛** | 连续熔断只发一次告警不重复推送，避免告警风暴。运维修复后手动重置熔断状态 |
| 空数据 | 当天跳过，不计入失败 |
...
| `"上涨 10"` 格式 | `10` | 中文描述正向变动 |
| `"下跌 5"` 格式 | `-5` | 中文描述负向变动 |
| `"+0.5%"` 百分比格式 | `0.5` | 百分比涨跌，百分比值固定为绝对值 |
| `"-1.2%"` 百分比格式 | `-1.2` | 同上 |
...
> ⚠️ URL 参数转义：跳转链接中的 `varietyName`、`area`、`province` 等参数必须经过 `encodeURIComponent()` 编码，禁止直接拼接用户输入。同时使用 `rel="noopener noreferrer"` 防止 window.opener 钓鱼攻击。
...
source 值              任务 ID
  │                      │
  ▼                      ▼
python main.py run liangdawang    ← 不传 --task_id，自动获取
  │
  ├─ 1. 加载模块（按优先级）：
  │     ① 优先从 SDK 内置加载
  │        import collectorsdk.collectors.liangdawang
  │        → collectorsdk/collectors/liangdawang.py（预置采集器）
  │     ② SDK 找不到 → 从后端下载
  │        GET /api/datasource/{code}/script?version=0
  │        → ~/.cache/collectors/{code}.py
  │     ③ --local 模式
  │        → dev/collectors/{code}.py
  │
  ...
  │     task_id=17（从 active-script 自动获取）
  ...
  │
  └─ 6. 执行记录写入 t_task_execution（关联 scriptId=17）
```
...
| 加载方式 | `main.py run liangdawang`（自动从 SDK 内置加载，无需下载） | `main.py run liangxin`（从后端下载脚本到 cache） |
...
**现象：** 粮达网偶尔修正昨日价格（如今天把昨天的 2510 改成 2500）。

**处理方式：**

| 层级 | 行为 | 说明 |
|------|------|------|
| `t_price` 数据库 | ✅ 自动覆盖修正 | 唯一键 `(date, variety, region, source)`，`ON DUPLICATE KEY UPDATE` 更新 price/change_val/remark |
| AI 问答 | ✅ 自动反映最新值 | `query_price` 工具查的是 `t_price` 最新数据，修正后用户查到的就是修正值 |
| 用户感知 | ❌ 无显式通知 | 用户不会收到"昨日价格已修正"的通知。如果需要，二期可加"昨日修正提醒"功能 |

**不需要处理的知识库条目：** 因为我们不做知识库条目，所以不存在"已发布的知识条目内容过时"的问题。价格始终是 `t_price` 里的最新值。
...
| 约束 | 规则 | 示例 |
|------|------|------|
| 品种范围 | `variety` 参数限定：玉米/小麦/进口粮/国产大豆/生猪，LLM 无法匹配合法品种时**不调用工具** | 传入"稻谷"不调用工具，不走 SQL |
| 品种降级兜底 | LLM 不走 SQL 后，走**RAG + 联网搜索**（`hybrid_search`），从网络结果获取信息，来源标签标注 `🌐 联网搜索` | 用户至少能得到参考信息，不是冰冷的"暂不支持" |
| 品种幻觉防护 | system prompt 追加：`精确价格仅来自 query_price 工具查询结果，如果品种不在枚举范围中不要编造价格` | LLM 不会从训练数据编造虚假价格 |
...
| 模型 | 用途 | 位置 | 部署方式 | 计费参考 |
|------|------|------|---------|---------|
| `Qwen/Qwen2.5-14B-Instruct` | LLM 回答生成 | `ai-qa-service` | SiliconFlow API | 输入 ¥0.7/百万 tokens，输出 ¥2.0/百万 tokens |
| `tongyi-embedding-vision-flash` | 知识条目多模态向量化 | Java 后端 | DashScope API | 按次计费 |
| `BAAI/bge-large-zh-v1.5` | RAG 文本向量化 | Python `embed.py` | 本地部署 | 免费 |

#### 新增工具调用对 LLM 成本的影响

| 场景 | 每次调用 tokens | 每次成本 | 说明 |
|------|---------------|---------|------|
| RAG 纯文本（当前） | ~4,500 | ~¥0.004 | 一轮 LLM 调用 |
| 工具调用价格查询（新增） | ~10,200 | ~¥0.013 | 两轮 LLM 调用（含 tools schema + tool_result） |
| 日均 1000 次，30% 走工具 | 平均 ~5,700 | ~¥0.006/次 | 混合场景 |

**月度成本对比：**

| 项目 | 当前 | 加上工具后 | 增幅 |
|------|------|-----------|------|
| 月度 LLM 成本（SiliconFlow） | ~¥3.2 | ~¥8 | +¥4.8 |
| 知识库多模态向量化（DashScope） | 不变 | 不变 | 不受影响 |
| RAG 文本向量化（BGE 本地） | ¥0 | ¥0 | 不受影响 |

> 成本增量极小（月增不到 ¥5），且工具调用仅在精确价格类问题时触发，
> 行情分析类问题仍走一轮 RAG。
>
> 当前 `token_used` 监控未启用（`token_used=0` 硬编码），建议在实施工具注册时一并实现真实 token 统计。
...
| **运维** | 执行记录崩溃兜底 | `report_start` 时 heartbeat_timeout=600s，后端定时扫描超时 running 记录自动标记 failed | 采集器崩溃后前端不显示永久"运行中" |
| | 重试机制 | 请求失败自动重试 3 次，指数退避（1s→3s→9s） | transient 网络故障不影响采集 |
...
| | 单请求超时 | httpx 单次请求超时 30s（connect=10s, read=30s） | 防止单个慢请求占满连接池 |
| | 采集总超时 | 单次采集总超时 10 分钟，超时自动终止并标记 failed | 防止 API hang 住堵塞采集进程 |
...
| 问题 | 处理方式 | 理由 |
|------|---------|------|
| 生猪区域映射歧义 | 保持现状，province=大区、region=省份 | 生猪的查询模式是"吉林猪价"而非"东北猪价"，当前设计反而方便 |
| 粮质标准差异化 | 优先取 remark 字段值，remark 为空时用全局文案 | 粮达网 API 仅有"二等散粮"级备注，无细粒度质检字段 |
| 异常价格白名单表 | 本期不做白名单配置表 | price>99999 或 price=0 的概率极低，触发后人工处理即可 |
| 批量冲突跳过策略 | 统一使用 `ON DUPLICATE KEY UPDATE` | 回填仅一次，194K 条行锁压力可控（分 500 批/批） |
| 数据权限控制 | 本期不做 | 无业务需求，所有价格数据公开 |
| 来源优先级规则 | LLM 自主决定 | 价格和文本分析不冲突时互补，冲突时 LLM 自行判断 |
| 统一告警渠道 | 现有日志 + 待接入 | 钉钉/飞书告警后续统一做，不是本项目范围 |
| 数据回滚功能 | 手工 SQL 修复 | 脏数据场景极少发生，前端回滚页面的开发成本远高于收益 |
| 配置中心 | 硬编码 + 配置文件 | 仅 5 个配置项（超时/间隔/并发/重试/阈值），不值得上配置中心 |
| 预置采集器版本升级 | 随 SDK 全量发布 | 仅 1 个预置采集器，不需要灰度 |
| 数据冷备份 | 依赖 MySQL 备份 | t_price 全量仅 39MB，MySQL 每日 mysqldump 即可覆盖 |
| 数据脱敏 | 不做 | 价格数据无敏感信息 |

以上项目在后续版本有明确需求时再单独排期。
