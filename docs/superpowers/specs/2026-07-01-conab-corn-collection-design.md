...
collect()
  ...
  │
  └── ④ for each (title, url) in pdf_links:
        │
        ├── 文件名 = url 最后一段
        ├── minio_path = f"conab/{year}/{filename}"
        │
        ...
        │   ├── 3 次重试，阶梯间隔
        │   ├── 404 → 跳过（日志记录）
        │   └── 全部失败 → 标记 failed，continue
        │
        ├── minio.put_object(minio_path, data, content_type="application/pdf")
        │
        ├── 提取日期范围
        │     # 从文件名中提取 "05-01-a-09-01-2026"
        │     date_range = _extract_date_range(filename)
...
3. 提取每个链接的 `href`（完整绝对 URL）和 `text`（显示文本）
4. 从文件名中正则提取日期范围
...
def _download_file(self, url, max_retries=3):
    for attempt in range(1, max_retries + 1):
        try:
            resp = requests.get(url, headers={"User-Agent": self._random_ua()}, timeout=15)
...
- **单 PDF 失败**：不阻断整体循环，记日志后 `continue`
- **页面抓取失败**：HTTP 异常 → 记录告警 → 返回失败计数
- **MinIO 上传异常**：捕获异常 → 记日志 → `continue`
- **submit_report 异常**：不影响采集流程，仅记日志
